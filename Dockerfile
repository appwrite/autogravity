...
COPY models/optional/ /tmp/focalnet-optional/
ARG REQUIRE_FOCALNET_MODEL=0
...
RUN if [ -f /tmp/focalnet-optional/focalnet-human.onnx ]; then \
      echo "59164c601c98cea3f62b25166710831dac63e1a872fc64767c65316ad5385439  /tmp/focalnet-optional/focalnet-human.onnx" | sha256sum -c - \
      && cp /tmp/focalnet-optional/focalnet-human.onnx /opt/models/focalnet-human.onnx; \
    elif [ "${REQUIRE_FOCALNET_MODEL}" = "1" ]; then \
      echo "focalnet-human.onnx is required for this build" >&2; \
      exit 1; \
    else \
      echo "FocalNet weights not bundled; MODEL_BACKEND=focalnet will fail at startup"; \
    fi
